...
async def test_register_write_read(dut):
    ...
    assert (read_value & 0x0F) == 0x01, f"CTRL mismatch: expected 0x01, got 0x{read_value & 0x0F:02X}"
    ...
    assert dut.tx_data.value == 0x42, f"tx_data should be 0x42, got 0x{int(dut.tx_data.value):02X}"
...
async def test_baud_rate_selection(dut):
    ...
    for sel, baud_name in baud_configs.items():
        ...
        # Verify baud_sel output
        assert dut.baud_sel.value == sel, f"baud_sel should be {sel}, got {dut.baud_sel.value}"
