...
module aes_sbox (
    ...
    output reg  [7:0] data_out      // Substituted byte
);

    // Forward S-Box lookup table (256 entries)
    // Based on multiplicative inverse in GF(2^8) followed by affine transformation
    always @(*) begin
        if (!inverse) begin
