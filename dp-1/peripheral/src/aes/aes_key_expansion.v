...
module aes_key_expansion (
    ...
    // S-Box for SubWord operation (reusing forward S-Box logic)
    function [7:0] sbox;
        input [7:0] data_in;
        begin
            case (data_in)
    ...
    // SubWord operation - apply S-Box to each byte of 32-bit word
    ...
    // RotWord operation - rotate word left by 1 byte
    ...
    reg [3:0] round_cnt;
    reg [31:0] w [0:43];  // Word array (44 words for 11 round keys)
    integer i;
        ...
        if (!rst_n) begin
            ...
            case (state)
                IDLE: begin
                    ...
                    if (start) begin
                        // Initialize first round key (K0) with master key
                ...
                EXPAND: begin
                    ...
                    if (round_cnt <= 10) begin
                        // Key expansion core operation
                        w[4*round_cnt]     = w[4*round_cnt - 4] ^ 
                                            (sub_word(rot_word(w[4*round_cnt - 1])) ^ 
                                            {rcon(round_cnt), 24'h0});
                        w[4*round_cnt + 1] = w[4*round_cnt - 3] ^ w[4*round_cnt];
                        w[4*round_cnt + 2] = w[4*round_cnt - 2] ^ w[4*round_cnt + 1];
                        w[4*round_cnt + 3] = w[4*round_cnt - 1] ^ w[4*round_cnt + 2];
                        
                        // Store round key
                        round_keys[round_cnt] <= {w[4*round_cnt], w[4*round_cnt + 1],
                                                 w[4*round_cnt + 2], w[4*round_cnt + 3]};
